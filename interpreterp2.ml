...
and pif () =
  ...
  let* i = ws >> many (cmd_parser () <|> pbegin () <|> pif ()) in 
  let* _ = ws >> keyword "Else" in
  let* e = ws >> many (cmd_parser () <|> pbegin () <|> pif ()) in 
  let* _ = ws >> keyword "End" in
  pure(If (fil i, fil e)) << ws

and pbegin () =
  let* _ = ws >> keyword "Begin" in
  let* content = ws >> many (cmd_parser () <|> pbegin () <|> pif ()) in
  let* _ = ws >> keyword "End" in
  pure(Begin (fil content)) << ws

and finalparser () = (many (cmd_parser () <|> pif () <|> pbegin ()))

let masterparser = finalparser ()

let sbegin =
  let* _ = ws >> keyword "Begin" in
  let* content = ws >> many (cmd_parser ()) in
  let* _ = ws >> keyword "End" in
  pure(Begin (fil content))

let sif =
  let* _ = ws >> keyword "If" in
  let* i = ws >> many (cmd_parser () <|> pbegin () <|> pif ()) in 
  let* _ = ws >> keyword "Else" in
  let* e = ws >> many (cmd_parser ()) in 
  let* _ = ws >> keyword "End" in
  pure(If (fil i, fil e))
